...
const socialLinks = [
  ...
  { icon: MessageCircle, href: "https://wa.me/919821020995", label: "WhatsApp", color: "from-amber-600 to-amber-500" },
];
...
function ContactItem({ href, icon: Icon, label, value, target, className = "" }) {
  return (
    <motion.a
      ...
      className={`flex items-center gap-4 p-4 glass-card border border-amber-500/15 hover:border-amber-400/40 transition-all duration-300 group ${className}`}
      whileHover={{ x: 4 }}
    >
      <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-300 shadow-md">
        <Icon size={18} className="text-white" />
      </div>
      <div className="flex-1 min-w-0">
        <div className="text-[11px] text-amber-300/60 font-semibold uppercase tracking-widest mb-0.5">{label}</div>
        <div className="text-sm md:text-base font-bold text-white group-hover:text-amber-100 transition-colors break-words">{value}</div>
      </div>
      <ArrowRight size={14} className="ml-auto text-amber-400/40 group-hover:text-amber-400 transition-colors flex-shrink-0" />
    </motion.a>
  );
}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
            ...
            <SlideIn from="left">
              <div className="space-y-5">
                <h2 className="text-xl font-black text-gradient-amber font-serif mb-6">Contact Information</h2>
                ...
                {/* Websites */}
                <div className="flex items-start gap-4 p-4 glass-card border border-amber-500/15 transition-all duration-300">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center flex-shrink-0 shadow-md mt-1">
                    <Globe size={18} className="text-white" />
                  </div>
                  <div className="flex-1 min-w-0 text-left">
                    <p className="text-xs text-amber-300/60 uppercase font-bold tracking-wider mb-2">Websites</p>
                    <div className="space-y-2 flex flex-col">
                      ...
                      <a
                        ...
                        className="text-sm font-semibold text-white hover:text-amber-300 transition-colors duration-300 break-all"
                      >
                        timecybermedia.com
                      </a>
                      <a
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    <ContactItem
                      ...
                      className="text-left"
                    />
                    <div className="w-full h-[220px] rounded-3xl overflow-hidden border border-amber-500/15 shadow-xl relative group">
                      <iframe
            ...
            <SlideIn from="right" delay={0.1}>
              <div className="space-y-6">
                ...
                {/* Social Media */}
                <div className="glass-card border border-amber-500/15 p-6">
                  <h3 className="text-lg font-black text-gradient-amber font-serif mb-2">Follow Us</h3>
                  <p className="text-amber-100/60 text-sm mb-5">Stay updated with the latest news, announcements, and highlights.</p>
                ...
                <div className="glass-card border border-amber-500/15 p-6">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-amber-200/70">Working Days</span>
                      <span className="text-white font-semibold">Mon – Sat</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-amber-200/70">Timing</span>
                      <span className="text-white font-semibold">10:00 AM – 6:00 PM</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-amber-200/70">Response Time</span>
                      <span className="text-amber-300 font-semibold">Within 1 business day</span>
                    </div>
                  </div>
                </div>
                {/* Phone Numbers */}
                <div className="glass-card border border-amber-500/15 p-6">
                  <h3 className="text-lg font-black text-gradient-amber font-serif mb-4">Phone Numbers</h3>
                  <div className="space-y-3">
                    {phones.map((phone, i) => (
                      <a
                        key={i}
                        href={`tel:${phone.tel}`}
                        className="flex items-center gap-3 p-3 rounded-xl hover:bg-amber-500/5 transition-all duration-300 group"
                      >
                        <div className="w-8 h-8 rounded-lg bg-amber-500/10 flex items-center justify-center text-amber-400 group-hover:scale-110 group-hover:bg-amber-500/20 transition-all duration-300">
                          <Phone size={14} />
                        </div>
                        <div>
                          <p className="text-[10px] text-amber-300/60 uppercase font-bold tracking-wider">{phone.label}</p>
                          <p className="text-sm font-semibold text-white group-hover:text-amber-300 transition-colors duration-300">{phone.display}</p>
                        </div>
                      </a>
                    ))}
                  </div>
                ...
                {/* Quick enquiry nudge */}
                <div className="glass-card border border-amber-400/20 p-6 bg-amber-900/10 animate-border-glow">
