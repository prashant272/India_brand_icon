...
export default function Login() {
  ...
  return (
    <PageHero
      ...
      icon="🔐"
      title="Portal Login"
      subtitle="India Brand Icon Award & Conference, 2026 — Access your dashboard and manage nominations."
    >
            ...
            <div className="glass-card p-8 md:p-10 border-white/10 shadow-2xl relative overflow-hidden group">
              ...
              <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
                <div className="space-y-2">
                  <label className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-amber-400/70 ml-1">
                    <FiMail /> Email Address
                  </label>
                  <input
                    type="email"
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-5 py-4 text-white text-sm placeholder:text-white/20 focus:outline-none focus:border-amber-500/50 focus:bg-white/10 transition-all"
                    value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    required
                    placeholder="name@india brand icon.com"
                  />
                </div>

                <div className="space-y-2">
                  <div className="flex justify-between items-center px-1">
                    <label className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-amber-400/70">
                      <FiLock /> Password
                    </label>
                    <Link to="/forgot-password" size="xs" className="text-[9px] font-black uppercase tracking-widest text-amber-100/30 hover:text-amber-400 transition-colors">
                      Forgot?
                    </Link>
                  </div>
                  <input
              ...
              <div className="mt-8 pt-6 border-t border-white/5 text-center relative z-10">
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                  Don't have an account? <Link to="/register" className="text-amber-400 hover:text-amber-300 transition-colors ml-1 underline decoration-amber-400/30">Create One</Link>
                </p>
              </div>
            </div>
          </ScaleIn>
