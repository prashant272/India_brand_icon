...
const steps = [
  ...
  },
  {
    ...
    points: [
      ...
      "Panel comprises industry experts and india brand icon professionals",
  ...
  return (
    <>
      <PageHero
        ...
        subtitle="A multi-stage, transparent, and fair evaluation framework — recognising only the most deserving organisations and individuals across india brand icon."
      >
        {/* Timeline Steps */}
                    ...
                    <ul className="space-y-2 flex-1">
                      {step.points.map((pt, j) => (
                        <li key={j} className="flex items-start gap-2">
                          ...
                          <span className="text-amber-100/70 text-sm leading-relaxed group-hover:text-amber-100/90 transition-colors">{pt}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 pb-24">
          <FadeUp className="text-center mb-10">
            <div className="w-20 h-px bg-gradient-to-r from-transparent via-amber-500/50 to-transparent mx-auto mb-6" />
            <h2 className="text-headline text-gradient-amber mb-2">Our Core Principles</h2>
          </FadeUp>

          <StaggerContainer className="grid grid-cols-2 md:grid-cols-4 gap-4" staggerDelay={0.1}>
            {principles.map((p, i) => (
              <StaggerItem key={i}>
                <NeonCard color="amber" className="!p-0">
                  <div className="p-5 text-center group h-full flex flex-col items-center">
                    ...
                    <p className="text-amber-200/60 text-xs leading-relaxed">{p.desc}</p>
                  </div>
                </NeonCard>
              </StaggerItem>
            ))}
        ...
        <FadeUp delay={0.3} className="mt-10 glass-card border border-amber-500/15 p-6 md:p-8 text-center">
          <h3 className="text-xl font-black text-gradient-amber font-serif mb-3">Why This Process Matters</h3>
          <p className="text-amber-100/70 leading-relaxed max-w-2xl mx-auto">
            Our rigorous and transparent selection process ensures awards are merit-based, truly credible, and that every winner enjoys international trust and respect in the india brand icon industry.
          </p>
        </FadeUp>
      </PageHero>
    </>
