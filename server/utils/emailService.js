...
export const sendOTPEmail = async (email, otp) => {
    const mailOptions = {
        from: `"India Brand Icon Awards" <${config.EMAIL.FROM}>`,
        to: email,
        subject: "Verify Your Email - India Brand Icon Awards",
        html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #d4af37; border-radius: 10px;">
        <h2 style="color: #d4af37; text-align: center;">Welcome to India Brand Icon Awards</h2>
        <p>Dear User,</p>
        ...
        <hr>
        <p style="font-size: 12px; text-align: center;">© India Brand Icon Awards 2026</p>
      </div>
    `,
    };

    try {
        // [DEV MODE] Log OTP to console so the developer can see it even if Brevo fails to deliver
        console.log(`\n=========================================`);
...
export const sendNominationSuccessEmail = async (email, nomineeName, credentials = null) => {
    ...
    const mailOptions = {
        from: `"India Brand Icon Awards" <${config.EMAIL.FROM}>`,
        to: email,
        subject: credentials ? "Nomination Received & Account Created - India Brand Icon Awards 2026" : "Nomination Received - India Brand Icon Awards 2026",
        html: `
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: auto; padding: 30px; border: 1px solid #d4af37; border-radius: 15px; background-color: #ffffff;">
        ...
        <p style="font-size: 12px; color: #888; text-align: center; margin-bottom: 0;">
            Sent by <strong>India Brand Icon Awards & Summit 2026</strong><br>
...
export const sendPasswordResetEmail = async (email, otp) => {
    const mailOptions = {
        from: `"India Brand Icon Awards" <${config.EMAIL.FROM}>`,
        to: email,
        subject: "Password Reset Code - India Brand Icon Awards",
        html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #d4af37; border-radius: 10px;">
...
export const sendNominationCredentialsEmail = async (email, name, password) => {
    const mailOptions = {
        from: `"India Brand Icon Awards" <${config.EMAIL.FROM}>`,
        to: email,
        subject: "Your Account Credentials - India Brand Icon Awards 2026",
        html: `
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: auto; padding: 30px; border: 1px solid #d4af37; border-radius: 15px; background-color: #ffffff;">
        <div style="text-align: center; margin-bottom: 25px;">
            <h1 style="color: #d4af37; margin: 0; font-size: 28px;">Welcome to India Brand Icon Awards!</h1>
        </div>
        ...
        <p style="font-size: 12px; color: #888; text-align: center; margin-bottom: 0;">
            © India Brand Icon Awards 2026<br>
            Organised by Prime Time Research Media Pvt. Ltd.
        </p>
      </div>
    `,
